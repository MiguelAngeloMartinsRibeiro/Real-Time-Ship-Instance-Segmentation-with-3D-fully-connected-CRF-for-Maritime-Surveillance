...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            p=self.params
            '''
            recalled = self.eval['recall']
            print("\nRecall  || iouThr=0       || %s " % recalled[0])
            print("Recall  || iouThr=0.05    || %s " % recalled[1])
            print("Recall  || iouThr=0.1     || %s " % recalled[2])
            print("Recall  || iouThr=0.15    || %s " % recalled[3])
            print("Recall  || iouThr=0.2     || %s " % recalled[4])
            print("Recall  || iouThr=0.25    || %s " % recalled[5])
            print("Recall  || iouThr=0.3     || %s " % recalled[6])
            print("Recall  || iouThr=0.35    || %s " % recalled[7])
            print("Recall  || iouThr=0.4     || %s " % recalled[8])
            print("Recall  || iouThr=0.45    || %s " % recalled[9])
            print("Recall  || iouThr=0.5     || %s " % recalled[10])
            print("Recall  || iouThr=0.55    || %s " % recalled[11])
            print("Recall  || iouThr=0.6     || %s " % recalled[12])
            print("Recall  || iouThr=0.65    || %s " % recalled[13])
            print("Recall  || iouThr=0.7     || %s " % recalled[14])
            print("Recall  || iouThr=0.75    || %s " % recalled[15])
            print("Recall  || iouThr=0.8     || %s " % recalled[16])
            print("Recall  || iouThr=0.85    || %s " % recalled[17])
            print("Recall  || iouThr=0.9     || %s " % recalled[18])
            print("Recall  || iouThr=0.95    || %s \n" % recalled[19])
            ...
            for tsh in range(0,20):
                ...
                for i in range(len(IoUcopy2)):
                    aux = IoUcopy2[i+1,1]
                    if len(aux) > 0:
                        nMaxDetections = nMaxDetections + aux.shape[0]
                        gtDetections = gtDetections + aux.shape[1]
                        
                        for x in range(aux.shape[0]):
                            if max(aux[x]) > p.iouThrs[tsh]:
                                TP = TP +1
                            else:
                                FP = FP +1
                    if TP + FP > 0:
                        ...
                        MyPrecision = 0
                print("Precision  || iouThr=%g       || %s " %(p.iouThrs[tsh],MyPrecision))
            '''
            ...
            for i in range(len(IoUcopy2)):
                aux = IoUcopy2[i+1,1]
                GTTT = len(self._gts[i+1,1])
                IOU = 0
                if len(aux) != 0:
                    for y in range(aux.shape[1]):
                        ioumax = max(aux[:,y])
                        if ioumax > 0.5:
                            iouvector2.append(ioumax)

            if (len(iouvector2)) > 0:
                IoUtotal = sum(iouvector2)/(len(iouvector2))
            
            #print("IoU  || iouThr=%g       || %s " %(p.iouThrs[tsh],IoUtotal))
            print("IoU | %s " %(IoUtotal))
            return stats
        if not self.eval:
