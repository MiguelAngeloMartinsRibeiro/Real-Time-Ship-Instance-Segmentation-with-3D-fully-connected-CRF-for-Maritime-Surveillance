...
cdef class DenseCRF3D(DenseCRF):

    # The same comments as in the superclass' `__cinit__` apply here.
    def __cinit__(self, int w, int h, int z, int nlabels, *_, **__):
        if type(self) is DenseCRF3D:
            self._this = self._this3d = new c_DenseCRF3D(w, h, z, nlabels)
    ...
    def addPairwiseGaussian(self, sxy, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        ...
        self._this3d.addPairwiseGaussian(sxy[0], sxy[1], sxy[2], _labelcomp(compat), kernel, normalization)

    def addPairwiseBilateral1Mod(self, sxy, sMod, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy, sxy)

        if isinstance(sMod, Number):
            sMod = (sMod)
        ...
        self._this3d.addPairwiseBilateral1Mod(
            sxy[0], sxy[1], sxy[2], sMod[0], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )

    def addPairwiseBilateral2Mod(self, sxy, sMod, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy, sxy)
        ...
        self._this3d.addPairwiseBilateral2Mod(
            sxy[0], sxy[1], sxy[2], sMod[0], sMod[1], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )
    
    def addPairwiseBilateral3Mod(self, sxy, sMod, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy, sxy)
        ...
        self._this3d.addPairwiseBilateral3Mod(
            sxy[0], sxy[1], sxy[2], sMod[0], sMod[1], sMod[2], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )

    def addPairwiseBilateral4Mod(self, sxy, sMod, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy, sxy)

        if isinstance(sMod, Number):
            sMod = (sMod, sMod, sMod, sMod)
        ...
        self._this3d.addPairwiseBilateral4Mod(
            sxy[0], sxy[1], sxy[2], sMod[0], sMod[1], sMod[2], sMod[3], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )

    def addPairwiseBilateral5Mod(self, sxy, sMod, unsigned char[:,:,::1] rgbim not None, compat, KernelType kernel=DIAG_KERNEL, NormalizationType normalization=NORMALIZE_SYMMETRIC):
        if isinstance(sxy, Number):
            sxy = (sxy, sxy, sxy)

        if isinstance(sMod, Number):
            sMod = (sMod, sMod, sMod, sMod, sMod)
        ...
        self._this3d.addPairwiseBilateral5Mod(
            sxy[0], sxy[1], sxy[2], sMod[0], sMod[1], sMod[2], sMod[3], sMod[4], &rgbim[0,0,0], _labelcomp(compat), kernel, normalization
        )